...
void Camera::Focus_Camera
(double focal_distance,double aspect_ratio,double field_of_view)
{
...
{
    vec3 worldPos;
    vec2 cx = Cell_Center(pixel_index);
    //vec2 Cy = Cell_Center(pixel_index)[1];
    worldPos = film_position + (this->horizontal_vector*cx[0]) + (this->vertical_vector*cx[1]);
    return worldPos;
}
